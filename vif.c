...
static void io_control(struct ancs_container *temp_vif){
	...
	k=temp_vif->weight;
	if(diff>0){
		dat = k*int_pow((diff + goal - 1)/goal,3);
		if(dat>MAX_DIFF)
			dat=MAX_DIFF;
		after = before + dat;
	}
	else {
		dat = k*int_pow((perf - goal + 1)/goal,3);
		if(dat>MAX_DIFF)
                        dat=MAX_DIFF;
		after = before - dat;
	}
...
void new_vif(struct net_bridge_port *p){
	...
	vif->weight = 5000;	//0 is arbitary value, give weight to check QOS algorithm working.
...
//	proc_vif[idx].file[3] = proc_create_data("resource_type",0600, proc_vif[idx].dir, &vif_opt, vif);
	proc_vif[idx].file[3] = proc_create_data("pid",0600, proc_vif[idx].dir, &vif_opt, vif);
